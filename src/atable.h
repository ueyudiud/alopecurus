...
intern Value ai_obj_vlookup(a_henv env, a_hobj self, a_enum tm);
...
always_inline Value ai_obj_vlookup_val(a_henv env, Value val, a_enum tm) {
	return v_is_obj(val) ? ai_obj_vlookup(env, v_as_obj(val), tm) : v_of_nil();
}
